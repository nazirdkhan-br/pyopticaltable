...
class Tools:
    ...
    def rotate_point(point, angle, origin):
        ...
        ----------
            point : tuple
                Coordinates (x,y) of the point to be rotated.
            angle : float
                Angle in degrees to rotate the point by.
            origin : tuple
                Coordinates of the axis of rotation. Default is (0,0).
        Returns
        ----------
            point_rot : tuple
        ...
        angle_rad = Tools.deg_to_rad(angle)
        
        x_rot = (point[0]-origin[0])*np.cos(angle_rad) - \
            (point[1]-origin[1])*np.sin(angle_rad) + origin[0]
        
        y_rot = (point[0]-origin[0])*np.sin(angle_rad) + \
            (point[1]-origin[1])*np.cos(angle_rad) + origin[1]
...
class OpticalElement:
    ...
    -----------
        ...
        mode : 
            mode of the optic (UNUSED) - transmissive "t", reflective "r",
            absorptive "a", or nonlinear "n" 
        angle : float
...
class OpticalTable:
    ...
    def beamsplitter_line(self, x, y, size, angle, direction, colour='k',
                          label=None, label_pos='top', labelpad=0.25, 
                          textcolour='k', fontsize=fontparams['fontsize']):
        """
        ...
        if not direction in ('L', 'R'):
            raise ValueError('Allowed values for direction are "L" and "R".')
        if direction == 'L':
            self.angled_line(x, y, size*np.sqrt(2), angle=angle+45, colour = 'gray')
        if direction == 'R':
            self.angled_line(x, y, size*np.sqrt(2), angle=angle-45, colour = 'gray')
        self.set_label(self.ax, x, y, size, label, label_pos,
                       labelpad, textcolour, fontsize=fontsize)
